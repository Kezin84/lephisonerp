      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="loading && !items.length">
            <td colspan="6" class="text-center py-8 text-muted">Đang tải dữ liệu...</td>
          </tr>
          <tr v-else-if="!items.length">
            <td colspan="6" class="text-center py-8 text-muted">Không có điều khoản nào.</td>
          </tr>
          <tr v-for="(item, index) in items" :key="item.id">
            <td class="text-center">{{ index + 1 }}</td>
            <td>
              <span class="badge bg-primary text-white" style="padding: 4px 8px; border-radius: 4px; display: inline-block;">{{ item.mau || 'Mặc định' }}</span>
            </td>
            <td>
            ...
            <td>
              <div class="noi-dung-text" v-html="item.noiDung"></div>
            </td>
        ...
        <div class="modal-body">
          <form @submit.prevent="saveItem" class="form-grid">
            ...
            <div class="form-group full-width">
              ...
              <ExcelEditor v-model="formData.noiDung" placeholder="Nhập nội dung điều khoản..." />
            </div>
          </form>
        </div>
